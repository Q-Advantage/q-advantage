...
3. **`main` and production gate.** Never push to `main`, never force-push, never merge your own PR, never run a production deploy command. Open PRs; the founder merges. Deploys flow from merge, not from you. Force-push and branch deletion on `main` are hard-blocked at the GitHub level. **"Require PR before merging" is not yet GitHub-enforced** — a safe bypass for the benchmark bot's direct push couldn't be verified this session (see `docs/adr/0001-forge-repo-topology.md`), so it's running in evaluate-only mode for now. Until that's resolved, this guardrail is enforced by `.claude/settings.json` (binds Claude Code only) and by the founder's own discipline — not by a GitHub-side block on human pushes yet.
